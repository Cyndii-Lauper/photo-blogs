...
export default function AdminBatchEditPanelClient({
  ...
  const { isUserSignedIn, selectedPhotoIds, setSelectedPhotoIds } =
    useAppState();
  ...
  const renderActions = () =>
    isTagging ? (
      ...
      <>
        {(selectedPhotoIds?.length ?? 0) > 0 && (
          <>
            <LoaderButton onClick={() => setTags('')} isLoading={isLoading}>
              Tag ...
            </LoaderButton>
            <DeletePhotosButton 
              disabled={isLoading}
        ...
        contentMain={
          <div className="flex flex-col gap-2">
            <Note
              ...
              cta={
                // eslint-disable-next-line max-len
                <div className="flex items-center gap-2.5">{renderActions()}</div>
              }
              spaceChildren={false}
              hideIcon
            >
              {isTagging ? (
                ...
                <div className="text-base">{renderPhotoText()}</div>
              )}
            </Note>
            {tagErrorMessage && (
